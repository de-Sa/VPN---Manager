...
const db = {
  get: async (k) => {
    try {
      const r = await window.storage.get("vpn:" + k);
  ...
  set: async (k, v) => {
    try {
      await window.storage.set("vpn:" + k, JSON.stringify(v));
    } catch {}
  },
};
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", marginBottom: "36px" }}>
          ...
          <h1 style={{ color: K.text, margin: "0 0 6px", fontSize: "24px", fontWeight: "700" }}>VPN Manager</h1>
          <p style={{ color: K.dim, margin: 0, fontSize: "13px" }}>Sistema de Gerenciamento de Tokens VPN</p>
        </div>
        ...
        <div style={{ marginTop: "24px", background: K.bg, borderRadius: "10px", padding: "14px", fontSize: "12px" }}>
          <div style={{ color: K.dim, fontWeight: "700", letterSpacing: "0.06em", marginBottom: "8px" }}>CONTAS DE DEMONSTRAÇÃO</div>
          <div style={{ color: K.dim }}>Admin: <span style={{ color: K.blue, fontFamily: "monospace" }}>admin / admin123</span></div>
          <div style={{ color: K.dim, marginTop: "4px" }}>Consultor: <span style={{ color: K.blue, fontFamily: "monospace" }}>joao / 123456</span></div>
        </div>
      </div>
    </div>
  );
...
function ConsultantsTab({ data, onSave, showToast, setConfirm }) {
  const [form, setForm] = useState({ nome: "", clientId: "", username: "", password: "" });
  const upd = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }));

  const addConsultant = () => {
    if (!form.nome.trim() || !form.clientId || !form.username.trim() || !form.password.trim()) {
      showToast("Preencha todos os campos.", "error");
      return;
    }
    ...
    const newCons = { id: consId, nome: form.nome.trim(), clientId: form.clientId };
    const newUser = { id: userId, username: form.username.trim(), password: form.password.trim(), role: "consultant", nome: form.nome.trim(), consultorId: consId };
    onSave("consultants", [...data.consultants, newCons]);
    onSave("users", [...data.users, newUser]);
    setForm({ nome: "", clientId: form.clientId, username: "", password: "" });
    showToast("Consultor adicionado!", "success");
  };

  const removeConsultant = (cons) => {
    const hasActiveToken = data.tokens.find((t) => t.consultorId === cons.id && t.status === "ocupado");
    if (hasActiveToken) {
      showToast("Consultor possui VPN ativa. Libere antes de remover.", "error");
      return;
    }
    setConfirm({
      message: `Remover o consultor "${cons.nome}" e seu acesso ao sistema?`,
      onConfirm: () => {
        onSave("consultants", data.consultants.filter((c) => c.id !== cons.id));
        onSave("users", data.users.filter((u) => u.consultorId !== cons.id));
  ...
  return (
    <div>
      <div style={{ background: K.card, borderRadius: "12px", padding: "22px", border: `1px solid ${K.border}`, marginBottom: "20px" }}>
        <h3 style={{ color: K.text, margin: "0 0 16px", fontSize: "15px" }}>Adicionar Novo Consultor</h3>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
          <div>
            <label style={{ display: "block", color: K.muted, fontSize: "11px", fontWeight: "700", marginBottom: "6px" }}>NOME COMPLETO</label>
            <input style={inp({ width: "100%" })} placeholder="Nome do consultor" value={form.nome} onChange={upd("nome")} />
          </div>
          <div>
            <label style={{ display: "block", color: K.muted, fontSize: "11px", fontWeight: "700", marginBottom: "6px" }}>CLIENTE</label>
            <select style={inp({ width: "100%" })} value={form.clientId} onChange={upd("clientId")}>
          ...
          <div>
            <label style={{ display: "block", color: K.muted, fontSize: "11px", fontWeight: "700", marginBottom: "6px" }}>LOGIN DE ACESSO</label>
            <input style={inp({ width: "100%" })} placeholder="Nome de usuário" value={form.username} onChange={upd("username")} />
          </div>
          <div>
            <label style={{ display: "block", color: K.muted, fontSize: "11px", fontWeight: "700", marginBottom: "6px" }}>SENHA DE ACESSO</label>
            <input style={inp({ width: "100%" })} type="password" placeholder="Senha" value={form.password} onChange={upd("password")} />
          </div>
        </div>
        <button style={btn(K.blue)} onClick={addConsultant}>+ Adicionar Consultor</button>
      </div>

      <div style={{ background: K.card, borderRadius: "12px", border: `1px solid ${K.border}`, overflow: "hidden" }}>
        <div style={{ padding: "16px 22px", borderBottom: `1px solid ${K.border}` }}>
          <span style={{ color: K.muted, fontSize: "12px", fontWeight: "700", letterSpacing: "0.06em" }}>
            CONSULTORES ({data.consultants.length})
          </span>
        </div>

        {data.consultants.length === 0 ? (
          <div style={{ padding: "30px", textAlign: "center", color: K.dim }}>Nenhum consultor cadastrado.</div>
        ) : data.consultants.map((cons, i) => {
          const client = data.clients.find((c) => c.id === cons.clientId);
          const user = data.users.find((u) => u.consultorId === cons.id);
          const activeToken = data.tokens.find((t) => t.consultorId === cons.id && t.status === "ocupado");
          return (
            <div key={cons.id} style={{
              display: "flex", alignItems: "center", padding: "14px 22px",
              borderBottom: i < data.consultants.length - 1 ? `1px solid ${K.border}` : "none",
            }}>
              <div style={{
                width: "36px", height: "36px", borderRadius: "50%", background: K.purple + "33",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: "14px", fontWeight: "700", color: K.purple, marginRight: "14px", flexShrink: 0,
              }}>
                {cons.nome.charAt(0)}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ color: K.text, fontWeight: "600", fontSize: "14px" }}>{cons.nome}</div>
                <div style={{ color: K.dim, fontSize: "12px", marginTop: "2px" }}>
                  {client?.nome || "—"} · login: <span style={{ color: K.muted, fontFamily: "monospace" }}>{user?.username || "—"}</span>
                  {activeToken && <span style={{ color: K.red, marginLeft: "8px" }}>· em uso: {activeToken.nome}</span>}
                </div>
              </div>
              <button onClick={() => removeConsultant(cons)} style={btn(K.red, { padding: "5px 12px", fontSize: "12px" })}>
                Remover
              </button>
            </div>
          );
